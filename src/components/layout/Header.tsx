                ...
                return (
                  ...
                  >
                    {hasChildren ? (
                      ...
                      <Link
                        href={item.href}
                        className={`inline-flex items-center gap-1 rounded-lg px-3 py-2 text-sm font-medium transition-colors text-gray-500 hover:text-[#0F2744] hover:bg-gray-50`}
                      >
                        {t(item.label)}
                      </Link>
                    )}
          ...
          <div style={{position:"fixed",top:0,right:0,bottom:0,zIndex:9999,width:280,background:"#fff",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
            ...
            {mainNav.map((item) => {
              ...
              return (
                <div key={item.label}>
                  {hasKids ? (
                    <button onClick={() => {}} style={{display:"block",width:"100%",textAlign:"left",padding:"14px 20px",fontSize:16,color:"#111",background:"none",border:"none",borderBottom:"1px solid #f3f4f6",fontWeight:600,cursor:"default"}}>{t(item.label)}</button>
                  ) : (
